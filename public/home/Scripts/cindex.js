...
$.extend(Core, {	
	...
	initSearchInput:function(){
		...
		cKeyWords.focus(function(){
		...
		}).autoSearch("/cinema/search_tip.html?city=" +  Core.curCity.spell + "&keywords={key}&callback={callback}", function(data){
			...
			if( key === v ){
				...
				for(; i<n; i++)
					arr[i] = {
						text : list[i].key,
						value : "/cinema/" + list[i].url + ".html#from=cinema.searchenter",
		...
		function searchHandler(txt){
			...
			window.location='/'+core.postData.city+'/cinema/category-ALL-area-0-type-0.html?keywords='+keys+'#from=cinema.search';
		}				
	},
	//修改地图悬浮文案
	titleFloat:function(){
	...
	htmlGoback:'<li class="cinema_goback last"><a href="/'+Core.curCity.spell+'/cinema/category-ALL-area-0-type-0.html">返回全部影院</a></li>',
	//分页时更新左侧列表信息
	updateList:function(isInitSplitPage){
		...
		var drg=document.createDocumentFragment(),
			...
			htmlTml=['<li cid="{cid}">',
						'<div class="c_name"><span class="bubble_red">{index}</span><a target="_blank" href="/cinema/{cid}.html#from=cinema.enter">{name}</a></div>',
		...
		for(var i=startIndex,n=1;i<endIndex;i++,n++){
			...
			if(s >= 1){
				if(cinema.isSeatSupport == 1){
					c_btn.html('<a href="/cinema/'+cinema.id+'.html?ticketType=0#from=cinema.enter" class="btn_e34551 btn_89_29" target="_blank">购&nbsp;&nbsp;票</a>');
				}else{					
					c_btn.html('<a href="/cinema/'+cinema.id+'.html?ticketType=1#from=cinema.enter" class="btn_e34551 btn_89_29" target="_blank">购&nbsp;&nbsp;票</a>');	
				}			
			}else if(!!cinema.movieList && cinema.movieList.length > 0){
				c_btn.html('<a href="/cinema/'+cinema.id+'.html?ticketType=1#pq=1&from=cinema.enter" class="btn_45a2e3 btn_89_29"  target="_blank">查看排期</a>');
			}else{
	...
	getCinemaList:function(){
		...
		core.post('/cinema/getJsonList.html',core.postData,function(err,rs){
		...
		var core=Core,cinema=marker.cinemaInfo,
			htmlTml=['<div class="cinema_map_info" cid="{cid}">',
            	...
            	'<div class="cinema_map_top">',
                	'<div class="c_name f14 fb"><a href="/cinema/{cid}.html?#from=cinema.enter" target="_blank">{name}</a></div>',
                    '<div class="c_price"><span><em>¥{lowPrice}</em><i>起</i></span></div>',
                    '<div class="c_add">地址：{address}</div>',
                    '<div class="c_tel">电话：{tel}</div>',
                '</div>',
